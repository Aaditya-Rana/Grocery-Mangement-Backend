...
export class EventsGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    handleSubscribe(@MessageBody() data: { listId: string }, @ConnectedSocket() client: Socket) {
    ...
    handleUnsubscribe(@MessageBody() data: { listId: string }, @ConnectedSocket() client: Socket) {
        const room = `list_${data.listId}`;
        client.leave(room);
